...
The training code must be self-sufficient. This means it cannot use external resources (e.g. files) that would have been generated after another (possibly longer) training phase. For the same reason, we recommend to stick to [numpy](https://numpy.org), [scipy](https://scipy.org) and [matplotlib](https://matplotlib.org)  dependencies (only).
